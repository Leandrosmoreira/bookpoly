...
MIN_BALANCE_USDC = 25.0       # Saldo mínimo (USDC) para maior mão (XRP23) @ ~99%
...
ASSET_PARAMS = {
    'eth': {'min_price': 0.600, 'entry_window_start': 80, 'entry_window_end': 20, 'stop_prob': 0.05, 'shares': 17},  # WR 58.2%
    'sol': {'min_price': 0.775, 'entry_window_start': 80, 'entry_window_end': 20, 'stop_prob': 0.05, 'shares': 19},  # WR 62.4%
    'xrp': {'min_price': 0.825, 'entry_window_start': 40, 'entry_window_end': 10, 'stop_prob': 0.05, 'shares': 23},  # WR 74.9%
}
...
def main():
    ...
    while _running:
        ...
        for asset in ASSETS:
            ...
            # 8. Enviar ordem
            if dry_run:
                log_event("DRY_RUN_ORDER", asset, ctx, side=side, price=price, size=ap['shares'])
